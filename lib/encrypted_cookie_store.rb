  ...
  module Session
    class EncryptedCookieStore < CookieStore
      ...
      def unpacked_cookie_data(env)
        env['encrypted_cookie_store.cookie'] ||= begin
          stale_session_check! do
            ...
            data ||= {}
            env['encrypted_cookie_store.original_cookie'] = data.deep_dup
            data
          end
        end
      end
      ...
      def session_changed?(env, session)
        session != env['encrypted_cookie_store.original_cookie']
      end

      def refresh_session?(env, options)
        if options[:refresh_interval] && time = timestamp(env)
